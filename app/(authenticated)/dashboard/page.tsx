...
interface Todo {
  ...
  completed: boolean;
  userId: string;
  createdAt: string; // JSON string from API
  updateAt: string;  // JSON string from API
}
  ...
  const fetchTodos = useCallback(
    async (page: number) => {
      ...
      try {
        ...
        // Convert dates to strings to match frontend Todo type
        const todosFromServer: Todo[] = data.todos.map((t: any) => ({
          id: t.id,
          title: t.title,
